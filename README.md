...
|-------|-------|
| **[프로젝트 관리]** |
| MVVM 패턴 도입 | MVVM 패턴 도입 및 디렉토리 규칙을 정함으로써 협업 프로세스 정리 |
| **[ 로그인 및 사용자 관리 ]** |
| 로그인 기능 | 카카오/구글/자체 로그인 - 각 플랫폼의 0Auth2 인증 프로세스를 분석하고 공통 인터페이스로 통합 |
| 자동 로그인 기능 | 사용자 토큰/로그인 정보를 SQLite에 저장. 앱 실행 시 스플래시 단계에서 자동 로그인 처리 |
| 마이페이지 기록 열람 기능 | 개인 러닝 및 배틀 기록을 SQLite에서 불러와 날짜별 조회 가능하도록 구현 |
| **[ 앱 - 서버 연동 ]** |
| REST API 연동, 데이터 매핑 | Retrofit2 기반 서버 통신 로직 작성 |
| 그리드 소유권 표시 오류 해결 | 서버 JSON 키 타입 불일치 문제를 분석/수정해 지도 소유권 에러 해결 |
| **[ 러닝 기능 ]** |
| 개닝 러닝 기능 | GPS 기반 거리/시간/경로 추적 및 러닝 종료 후 결과 요약 화면 구성 |
| 배틀 러닝 기능 | 매칭된 두 유저 간 실시간 영역 점령 기능 및 소유권 상태 실시간 갱신 |
...
|-------|-------|
| 전체 UI 디자인 | - |
| 전체 UI 구현 | 홈, 배틀, 러닝 결과, 마이페이지 등 주요 화면의 전체 레이아웃 및 인터랙션 구현 |
