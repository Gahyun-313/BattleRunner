...
import com.example.battlerunner.databinding.FragmentHomeBinding // 뷰 바인딩을 위한 FragmentHomeBinding 임포트
...
class HomeFragment : Fragment(R.layout.fragment_home), OnMapReadyCallback { // HomeFragment 클래스 선언 및 OnMapReadyCallback 구현

    private var _binding: FragmentHomeBinding? = null // 뷰 바인딩을 위한 변수 선언
    private val binding get() = _binding!! // 바인딩 객체 접근을 위한 getter
    private var startTime: Long = 0 // 타이머 시작 시간을 저장할 변수 선언
    private var elapsedTime: Long = 0 // 경과 시간을 저장할 변수 선언
    private var handler: Handler = Handler(Looper.getMainLooper()) // 타이머 UI 업데이트를 위한 핸들러 선언
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        childFragmentManager.beginTransaction()
            .replace(R.id.fragmentContainer, supportMapFragment) // fragmentContainer에 지도 프래그먼트 설정
            .commit() // 트랜잭션 커밋
    ...
    private fun stopTimerAndMoveToNextScreen() {
        if (isRunning) { // 타이머가 실행 중일 때만 실행
            ...
            val intent = Intent(requireActivity(), PersonalEndActivity::class.java) // PersonalEndActivity로 이동할 인텐트 생성
            intent.putExtra("elapsedTime", elapsedTime) // 경과 시간 전달
            intent.putParcelableArrayListExtra("pathPoints", ArrayList(pathPoints)) // 경로 전달
            startActivity(intent) // PersonalEndActivity 시작
        }
    }
    ...
    private fun startLocationUpdates() {
        val locationRequest = LocationRequest.create().apply { // 위치 요청 생성 및 설정
        ...
        if (LocationUtils.hasLocationPermission(requireContext())) { // 위치 권한 확인
            try {
                fusedLocationClient.requestLocationUpdates(locationRequest, locationCallback, Looper.getMainLooper()) // 위치 업데이트 요청
            } catch (e: SecurityException) { // 권한 누락 시 예외 처리
                e.printStackTrace() // 예외 발생 시 스택 트레이스 출력
            }
        } else {
            LocationUtils.requestLocationPermission(this) // 권한이 없으면 요청
    ...
    companion object {
        const val LOCATION_PERMISSION_REQUEST_CODE = 1 // 위치 권한 요청 코드 정의
    }
}
